...
class MultiAPIManager:
    ...
    async def get_health_status(self) -> Dict[str, Dict]:
        ...
        for api_name, api in self.apis.items():
            health_status = self.health_monitor.get_health_status(api_name)
            if health_status:
                health_statuses[api_name] = health_status
            else:
                # Fallback if health monitor doesn't have status
                health_statuses[api_name] = {
                    "status": api.health_status,
                    "last_check": datetime.now(),
                    "rate_limits": api.rate_limiter.get_status()
                }
